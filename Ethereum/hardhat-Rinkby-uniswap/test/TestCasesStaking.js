...
describe("Uniswap test contract", function () {
  ...
  var TokenA, TokenB, TokenB, Liquidity, factoryA, pairAB, pairBC;

  var hardhatTokenA, hardhatTokenB, hardhatTokenC, hardhatLiquidity;
  ...
  it("3) Add1 should be able to swap TokenA for TokenB from pool", async function () {
    ...
    await hardhatTokenA.connect(addr1).approve(hardhatLiquidity.address, amountIn);
    
    //getting reserves so that we can calculate expected TokenB amount.
    var reserveTx = await hardhatLiquidity.getReservesExactIn(hardhatTokenA.address, hardhatTokenB.address);
    ...
    var reserveTx = await hardhatLiquidity.getReservesExactIn(hardhatTokenB.address, hardhatTokenC.address);
    var reciept = await reserveTx.wait();
    var event = reciept.events.find((event) => event.event === "getReservesAB");
    ...
    var tx = await hardhatLiquidity.connect(addr1).swapToken(
      amountIn,
      amountOutMin,
